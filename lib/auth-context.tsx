...
export interface AuthContextType {
  ...
  isLoading: boolean;
  isSignedIn: boolean;
  signUp: (email: string, password: string, name: string, phone?: string, sinpeTelefono?: string) => Promise<void>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signUp = async (email: string, password: string, name: string, phone?: string, sinpeTelefono?: string) => {
    const baseUrl = getApiBaseUrl();
    const response = await fetch(`${baseUrl}/api/auth/signup`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, password, name, ...(phone && { phone }), ...(sinpeTelefono && { sinpeTelefono }) }),
      credentials: "include",
    });
